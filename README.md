...
新版当前包含 `/`、`/learn`、`/neetcode`、`/discover` 四条路由，以及 Python 工作草稿、Draft/Stuck/Solved Attempt 和本地 Todo 状态 API。`/learn` 内嵌 freeCodeCamp 官方 React 视频和 Advanced TypeScript 播放列表，学习次数与笔记仅保存在这台 Mac 的浏览器 localStorage。`/discover` 是只读湾区活动雷达：每天从 Brave 搜索 Luma、CMU Silicon Valley 和大厂在 Bay Area 的活动候选，并提供稳定的官方活动入口；不会自动报名或向外部平台回写。后端测试使用 `.venv/bin/pytest`，前端检查使用 `npm run typecheck`、`npm test` 和 `npm run build`。
